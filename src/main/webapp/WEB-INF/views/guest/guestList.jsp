...
<div class="container">
  ...
  <table class="table table-borderless m-0 p-0">
  	<tr>
  	  ...
  	  <td><a href="insertDummy" class="btn btn-success btn-sm">더미 게시글 생성</a></td>
  	  <td class="text-end">
  	...
  	<tr>
  	  <td colspan="2" class="text-end">
  	    <c:if test="${pag > 1}">
  	      <a href="guestList?pag=1" title="첫페이지" class="text-decoration-none">첫페이지로 이동</a>
  	    ...
  	    <c:if test="${pag < totPage}">
  	      ...
  	      <a href="guestList?pag=${totPage}" title="마지막페이지" class="text-decoration-none">마지막페이지로 이동</a>
  	    </c:if>
  	  </td>
  	</tr>
  </table>
...
<div class="container2 text-center mt-3">
    ...
    <ul class="pagination justify-content-center">
        <c:if test="${pag > 1}">
            <li class="page-item">
                <a class="page-link" href="guestList?pag=1" aria-label="First">&laquo;&laquo;</a>
            </li>
        </c:if>
        ...
        <c:if test="${curBlock > 0}">
            <li class="page-item">
                <a href="guestList?pag=${(curBlock-1)*blockSize+1}" class="page-link">Previous</a>
            </li>
        </c:if>
        ...
        <c:forEach var="i" begin="${(curBlock*blockSize)+1}" end="${(curBlock*blockSize+blockSize)}" varStatus="st">
            ...
            <c:if test="${i <= totPage && i == pag}">
                <li class="page-item active" aria-current="page">
                    <a class="page-link" href="guestList?pag=${i}">${i}</a>
                </li>
            </c:if>
        ...
        <!-- 다음페이지 -->
        <li class="page-item">
            <a class="page-link" href="guestList?pag=${(curBlock+1)*blockSize+1}">Next</a>
        </li>

        <!-- 마지막 페이지 -->
        <c:if test="${pag < totPage}">
            <li class="page-item">
                <a class="page-link" href="guestList?pag=${totPage}">&raquo;&raquo;</a>
            </li>
        </c:if>
    </ul>
</div>
